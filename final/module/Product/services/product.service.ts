...
export default class ProductService {
    ...
    searchProductByTitle = (payload: string) => {
        return new Promise<any>(async (resolve, reject) => {
            try {
                console.log(payload);
                let productLst = await Product.find({ name: { $regex: new RegExp('^|.*\\s+' + payload + '\\s+.*|$', 'i') } }).limit(3).exec();
    ...
    updatePaymentStatus = (id: string) => {
        return new Promise<any>(async (resolve, reject) => {
            try {
                const data = await Order.findOneAndUpdate({ _id: id }, { payment_status: true }, { new: true })
                if (data) {
