    ...
    constructor() {
        ...
        this.searchProductByTitle = (payload) => {
            return new Promise(async (resolve, reject) => {
                try {
                    console.log(payload);
                    let productLst = await product_model_1.Product.find({ name: { $regex: new RegExp('^|.*\\s+' + payload + '\\s+.*|$', 'i') } }).limit(3).exec();
                    ...
                    const error = {
                        message: 'Could not find anything matched with input!',
                        success: false,
                    };
                    resolve(error);
                }
        ...
        this.getOrders = (id) => {
            return new Promise(async (resolve, reject) => {
                try {
                    const data = await product_model_1.Order.find({ userId: id });
                    if (data) {
                        const response = {
                            success: true,
        ...
        this.updatePaymentStatus = (id) => {
            return new Promise(async (resolve, reject) => {
                try {
                    const data = await product_model_1.Order.findOneAndUpdate({ _id: id }, { payment_status: true }, { new: true });
                    if (data) {
                        const response = {
                            success: true,
                            message: 'Trạng thái thanh toán đã được cập nhật!'
                        };
                        resolve(response);
                    }
                    const error = {
                        success: false,
                        message: 'Không thể cập nhật trạng thái thanh toán!'
                    };
                    resolve(error);
                }
                catch (error) {
